        ...
        stage('Pull Docker Images') {
            parallel {
                ...
                stage('Pull Booking Image') {
                    steps {
                        sh 'docker pull radum1977/mybookingapi:1.0'
                    }
                }
            }
        }
        ...
        stage('Run API Test Cases in Parallel') {
            parallel {
                stage('Run GoRest Tests') {
                    steps {
                        sh 'docker run --rm -v $(pwd)/newman:/app/results radum1977/gorestddtest:1.0'
                    }
                }
                stage('Run Booking Tests') {
                    steps {
                        sh 'docker run --rm -v $(pwd)/newman:/app/results radum1977/mybookingapi:1.0'
                    }
                }
            }
        }
